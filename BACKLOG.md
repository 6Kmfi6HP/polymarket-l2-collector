...
| 功能 | 来源 | 状态 | 测试结论 |
|------|------|------|----------|
| hftbacktest 事件格式转换器：将 Parquet 导出数据转换为 hftbacktest 兼容的 numpy event_dtype 数组（orderbook/trade 转换、时间戳处理、事件顺序修正 `correct_event_order`、负延迟修正 `correct_local_timestamp`、事件验证 `validate_event_order`、市场结算 `--settlement`、CLI 入口 `polymarket-hbt-convert`；84 条测试覆盖） | pm-hftbacktest `polymarket_to_hbt()` + `correct_event_order()` + `correct_local_timestamp()` + `validate_event_order()` | 待测 | |
