...
| 功能 | 来源 | 状态 | 测试结论 |
|------|------|------|----------|
| hftbacktest 事件格式转换器：将 Parquet 导出数据转换为 hftbacktest 兼容的 numpy event_dtype 数组（event_dtype、时间戳转换、事件顺序修正、CLI 入口 `polymarket-hbt-convert`） | pm-hftbacktest `polymarket_to_hbt()` + `correct_event_order()` + `event_dtype` | 待测 | |
